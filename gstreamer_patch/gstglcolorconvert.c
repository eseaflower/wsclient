...
/* BT. 709 full-range
 */
 // The offset is subtracted (added with a minus in front) before the matrix
 // calculations. The values (YBR) we get are from a GL-texture, with 8-bits
 // per channel, which means that the precision of the values are limited to
 // normalized 8-bit floats. The nominal offset, 0.5, cannot be represented
 // accurately in 8 bits, so the subtraction will underflow just a smidge.
 // This leads to a green tint on greyscale values, which are of primary concern
 // to us! If we use the 8-bit truncated value of 0.5 instead we can get accurate
 // reconstructions of grey values.
static const gfloat truncated_8bit_offset = 0.49804f;
static const gfloat from_yuv_bt709_full_offset[] = {-0.0f, -0.49804, -0.49804};
static const gfloat from_yuv_bt709_full_rcoeff[] = {1.0f, 0.0f, 1.5748f};
static const gfloat from_yuv_bt709_full_gcoeff[] = {1.0f,-0.187324f,-0.468124f};
static const gfloat from_yuv_bt709_full_bcoeff[] = {1.0f, 1.8556f, 0.000f};
...
/* BT. 709 full-range
 */
static const gfloat from_rgb_bt709_full_offset[] = {0.0f, 0.5f, 0.5f};
static const gfloat from_rgb_bt709_full_ycoeff[] = {0.2126f, 0.7152f, 0.0722f};
static const gfloat from_rgb_bt709_full_ucoeff[] = {-0.114572f, -0.385428f, 0.5f};
static const gfloat from_rgb_bt709_full_vcoeff[] = {0.5f, -0.454153f, -0.045847f};
...
_YUV_to_RGB (GstGLColorConvert * convert)
{
  ...
  if (gst_video_colorimetry_matches (&convert->in_info.colorimetry,
          GST_VIDEO_COLORIMETRY_BT709)) {
            // GST_ERROR("709_dec full, corr");
    printf("OBSERVE: Using special BT709 conversion. This assumes that the video is full range!. It will increase accuracy on grey scale.\n");
    ...
    info->cms_offset = (gfloat *) from_yuv_bt709_full_offset;
    info->cms_coeff1 = (gfloat *) from_yuv_bt709_full_rcoeff;
    info->cms_coeff2 = (gfloat *) from_yuv_bt709_full_gcoeff;
    info->cms_coeff3 = (gfloat *) from_yuv_bt709_full_bcoeff;
  } else {
    /* defaults/bt601 */
...
_RGB_to_YUV (GstGLColorConvert * convert)
{
  ...
  if (gst_video_colorimetry_matches (&convert->in_info.colorimetry,
          GST_VIDEO_COLORIMETRY_BT709) || gst_video_colorimetry_matches(&convert->out_info.colorimetry, GST_VIDEO_COLORIMETRY_BT709)) {
    printf("OBSERVE: Colorimetry might have been decided by output format!\n");
    printf("OBSERVE: Using full-range BT709 conversion.\n");
    ...
    info->cms_offset = (gfloat *) from_rgb_bt709_full_offset;
    info->cms_coeff1 = (gfloat *) from_rgb_bt709_full_ycoeff;
    info->cms_coeff2 = (gfloat *) from_rgb_bt709_full_ucoeff;
    info->cms_coeff3 = (gfloat *) from_rgb_bt709_full_vcoeff;
  } else {
    /* defaults/bt601 */
